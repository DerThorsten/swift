...
class EmscriptenSwift(cmake_product.CMakeProduct):
    @classmethod
    ...
    def should_build(self, host_target):
        # return True
        return self.args.build_emscriptenswift
    ...
    def _build(self, host_target, target_triple, short_triple):
        # cmark
        cmark_build_dir = self._build_cmark(host_target, target_triple, short_triple)

        llvm_build_dir = self._build_llvm(host_target, target_triple, short_triple)
        llvm_cmake_dir = os.path.join(llvm_build_dir, 'lib', 'cmake', 'llvm')
        self._build_it(host_target, target_triple, short_triple, llvm_cmake_dir)

    
    def _build_cmark(self, host_target, target_triple, short_triple):
        build_root = os.path.dirname(self.build_dir)
        cmark_build_dir = os.path.join(
            build_root, 'cmark-%s' % short_triple)
        cmark_source_dir = os.path.join(
            os.path.dirname(self.source_dir), 'cmark')
        ...
        # install prefix to WASM_PREFIX
        wasm_prefix = os.environ.get('WASM_PREFIX')
        if not wasm_prefix:
            raise Exception("WASM_PREFIX environment variable is not set")
        cmake_options.define('CMAKE_INSTALL_PREFIX:PATH', wasm_prefix)


        cmark_cmake = cmake.CMake(
            self.args, self.toolchain,
            prefer_native_toolchain=not self.args.build_runtime_with_host_compiler)
        shell.call(["env", self.toolchain.cmake, "-B", cmark_build_dir]
                   + list(cmark_cmake.common_options(self))
                   + list(cmake_options)
                   + [cmark_source_dir])
        
        shell.call(["env", self.toolchain.cmake, "--build", cmark_build_dir])

        # install cmark to WASM_PREFIX so that it can be found by llvm and swift build
        shell.call(["env", self.toolchain.cmake, "--install", cmark_build_dir])

        return cmark_build_dir

    def _build_llvm(self, host_target, target_triple, short_triple):
        ...
        # we need to enable clang
        cmake_options.define('LLVM_ENABLE_PROJECTS:STRING', 'clang')
        ...
        # also cross compile llvm!
        shell.call(["env", self.toolchain.cmake, "--build", build_dir])
    ...
    def _build_it(self, host_target, target_triple, short_triple, llvm_cmake_dir):

        print(f'host_target: {host_target}, target_triple: {target_triple}, short_triple: {short_triple}')
        # sys.exit(1)
        ...
        print("build runtime with host compiler: " + str(self.args.build_runtime_with_host_compiler))
        if self.args.build_runtime_with_host_compiler:
            self.cmake_options.define(
            ...
            print("native toolchain path: " + self.native_toolchain_path(host_target))
        ...
        # set clang_DIR
        print("llvm cmake dir: " + llvm_cmake_dir)
        clang_cmake_dir = os.path.join(llvm_cmake_dir, '..', 'clang')
        # make absolute path
        clang_cmake_dir = os.path.abspath(clang_cmake_dir)
        print("clang cmake dir: " + clang_cmake_dir)
        self.cmake_options.define('Clang_DIR:PATH', clang_cmake_dir)



        # set UUID_INCLUDE_DIR UUID_LIBRARY
        wasm_prefix = os.environ.get('WASM_PREFIX')
        if not wasm_prefix:
            raise Exception("WASM_PREFIX environment variable is not set")
        uuid_include_dir = os.path.join(wasm_prefix, 'include')
        uuid_library = os.path.join(wasm_prefix, 'lib', 'libuuid.a')
        self.cmake_options.define('UUID_INCLUDE_DIR:PATH', uuid_include_dir)
        self.cmake_options.define('UUID_LIBRARY:PATH', uuid_library)

        # // zstd_LIBRARY zstd_INCLUDE_DIR)
        zstd_include_dir = os.path.join(wasm_prefix, 'include')
        zstd_library = os.path.join(wasm_prefix, 'lib', 'libzstd.a')
        self.cmake_options.define('zstd_INCLUDE_DIR:PATH', zstd_include_dir)
        self.cmake_options.define('zstd_LIBRARY:PATH', zstd_library)
        ...
        # cmark_DIR
        cmark_cmake_dir = os.path.join(wasm_prefix, 'lib', 'cmake', 'cmark-gfm')
        print("cmark cmake dir: " + cmark_cmake_dir)
        assert os.path.exists(cmark_cmake_dir), "cmark cmake dir does not exist: " + cmark_cmake_dir
        assert "/Users/thorstenbeier/micromamba/envs/swift-wasm/lib/cmake/cmark-gfm" == cmark_cmake_dir, "cmark cmake dir is not correct: " + cmark_cmake_dir
        self.cmake_options.define('cmark_DIR:PATH', cmark_cmake_dir)



        # set cmake-build-type to release 
        self.cmake_options.define('CMAKE_BUILD_TYPE:STRING', 'Release')
        ...
        self.cmake_options.define(
            'SWIFT_STDLIB_INSTALL_ONLY_CLANG_RESOURCE_HEADERS:BOOL', 'FALSE')
